...
class Quiz extends Component {
  ...
  componentDidMount = () => {
    ...
    db.collection("quizes")
      ...
      .then(snapshot => {
        ...
        Object.keys(snapshot.data().questions).forEach(
          key => (emptyUserAnswers[key] = [])
        );
        ...
        Object.keys(questions).forEach(key => {
          ...
          while (spotsToFill > 0) {
            ...
            questions[key].displayOrder[spotsToFill] =
              questions[key].displayOrder[index];
  ...
  handleAnswerChange = (e, questionKey) => {
    ...
    this.setState(state => {
      ...
      checked
        ...
        : (newUserAnswers[questionKey] = newUserAnswers[questionKey].filter(
  ...
  handleSubmit = e => {
    ...
    if (Object.values(userAnswers).find(answers => answers.length === 0)) {
      ...
      this.setState({
        missingAnswerMsg:
          "You need to answer all questions before submitting the quiz"
      });
    } else {
      ...
      Object.keys(questions).forEach(questionKey => {
        ...
        const questionMaxPoints =
          Object.keys(questions[questionKey].correctAnswers).length *
          questions[questionKey].pointsPerAnswer;
        maxPoints += questionMaxPoints;

        // Calc questions score based on how many points each answer is worth
        let questionScore = userAnswers[questionKey].reduce(
          (score, answerKey) => {
